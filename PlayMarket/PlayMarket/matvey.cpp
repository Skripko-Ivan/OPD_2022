			...
			for (int i = 1; i <= 8; i++)
					...
					&& Mouse::isButtonPressed(Mouse::Left))
				{
					...
					if (flag == 1)//если просматриваются скачанные
					{
						...
						show_info(window, load + 1);
						break;
					}
					//если смотрится каталог и приложение не скачано
					show_info(window, start_app + i);
				}
	...
	while (!(Keyboard::isKeyPressed(Keyboard::Escape)))
	{
		...
		if (Mouse::isButtonPressed(Mouse::Left))
			if (menuNum > 0 && menuNum < 3)
			{
				if (menuNum == 1)
				{
					give_rating(window, index);
					window.clear(Color(129, 181, 221));
					return;
				}
				else
				{
					comments(window, index);
				}
			}
...
void give_rating(RenderWindow& window, int index)
{
	...
	change_status_rating(index);//ставим флаг, что рейтинг дан
	std::string name, cnt, rating;
	std::ifstream load_file("rating.txt");
	for (int i = 1; i < index; i++)
	{
